					...
					<div class="good-item" v-for="item in goodsList" :key="item.goodid">
							<div class="item-card">
									<div class="item-content">
											...
											<!-- 卖家信息 -->
											<div class="cell seller">卖家：{{ item.username }}</div>
									</div>
							</div>
					</div>
			</div>
...
<script setup>
	...
	// 获取已买入的商品（ing=1 且买家用户名匹配）
	const fetchGoods = async () => {
			try {
					loading.value = true;
					const response = await api.getSoldGoodsByBuyer(props.username);
					goodsList.value = response.data.data;
			} catch (error) {
					console.error('获取已买入商品失败:', error);
			} finally {
					loading.value = false;
			}
	};
